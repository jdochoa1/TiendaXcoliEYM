...
//$sql = mysql_query("Select cons from feria_detal WHERE (fecha LIKE '2017-02-12%') group by cons order by cons") or die(mysql_error());
?>
<html>
<head>
...
Resolucion DIAN 320001394794<br>
del 1 al 5000 Abril 29 de 2016<br>
...
$sql_detal = mysql_query("select codigoP, nombreP, cantidad, valorT from FERIA_DETAL where cons = '".$row[0]."'") or die(mysql_error());
while ($sql_d = mysql_fetch_array($sql_detal)){
	echo "<tr><td>".$sql_d[0]." ".$sql_d[1]."</td><td align = 'right'>".$sql_d[2]."</td><td align = 'right'>$".number_format($sql_d[3])."</td></tr>";
$sub = $sub + $sql_d[3];
}
$iva = $sub * 0.19;
$tot = $sub + $iva;
