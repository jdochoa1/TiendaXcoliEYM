...
function factura(str){
	...
	if(u==''||v==''||x==''||y==''||z==''||a==''){
		...
		document.location.href="factura.php?a="+a+"&b="+b;
	}
}
</script>
<script type="text/javascript">
function bolsas(){
	...
	var impbolsa = 30 * bolsa;
	document.getElementById("impbol").value = 30 * bolsa;
	document.getElementById("grantotal").value = parseInt(total) + parseInt(impbolsa);
	document.writeln (_SESSION['bolsa']);
...
@$_SESSION['iva'] = (array_sum(@$_SESSION['vector_tot'])*19)/100;
@$_SESSION['bolsa'] = $_SESSION['bolsa'];
@$_SESSION['TOTAL'] = array_sum(@$_SESSION['vector_tot']) + @$_SESSION['iva'] + @$_SESSION['bolsa'];

?><input type="hidden" id="total" name = "total" value= <?php echo $_SESSION['TOTAL'];?>><?php
}	
}else{ ?>
...
echo "<tr><td colspan = '5'></td><td><b>IMP. CONS. BOLSA PLAST</b></td><td align = 'right'>$".@number_format($_SESSION['bolsa'],0)."</td><td></td></tr>";
echo "<tr height = '5'><td colspan = '5'></td><td><b>TOTAL</b></td><td align = 'right'>$<input type = 'text' id= 'grantotal' class='sinborde' name = 'grantotal' value = 0 size = '4' style = 'border:0 ; text-align:right;'  readonly ></td><td></td></tr>";
echo "</table>";
}
?>
</body>
