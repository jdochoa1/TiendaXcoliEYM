...
$fd = fopen("VENTAS.csv","w"); // lo que hago es crear un archivo de texto llamado archivo.txt	
	$l = "CONSECUTIVO;CLIENTE;NOMBRE;FECHA;DIRECCION;TELEFONO;EMAIL;TIPO PAGO;CODIGO;DESCRIPCION;VALOR;";
	fwrite ($fd, $l.''); //aqui estoy escribiendo en el archivo.txt el valor de la variable $l	
	$sqlfile = mysql_query("SELECT * FROM $VENTAS") or die (mysql_error());
	while($row = mysql_fetch_array($sqlfile)){	
	$l = "\n".$row[1].";". $row[2].";". $row[3].";". $row[4].";".$row[5].";".$row[6].";".$row[7].";".$row[8].";".$row[9].";".$row[10].";".$row[12].";";
	fwrite ($fd, $l.'');
	}	
	fclose($fd); //aqui estoy cerrando el archivo.txt
...
WHILE ($row_file = mysql_fetch_array($sqlfile)){
	...
	$l = "3".$row_file[0]."|". $row_file[1]."|". $row_file[2]."|".$row_file[3]."|".$row_file[4]."|".$row_file[5]."|".$row_file[6]."|".$row_file[7]."|".$row_file[8]."|"
	...
	$l = "3".$row[0]."|". $row[1]."|3".$row[2]."|".$row[3]."|".trim($row[4])."|".$row[5]."|".$row[6]."|".$row[7]."|".$row[8]."|".
...
function show_fac(str){
	var c = document.getElementById("factura").value;
	...
	xmlhttp.open("GET","listafacturas.php?c="+c,true);
	xmlhttp.send();
}
...
<div id = "inventario">
	...
	<tr height = '90'>
		...
		<td colspan = '4'>
			<input type "text" id="factura" name="factura" size = "60" OnChange = "show_fac(this.value);" />
		</td>
	</tr>
