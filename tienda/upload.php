...
$archivo=file($_FILES["archivo"]["name"]);
			...
			mysql_query($orden);}
			if ($_FILES["archivo"]["name"]=='baseitem.csv'){
			$orden="TRUNCATE $BASEITEM";	
			mysql_query($orden);}
    foreach($archivo as $linea){
	if ($_FILES["archivo"]["name"]=='productos.csv' || $_FILES["archivo"]["name"]=='baseitem.csv'){
	...
	if($_FILES["archivo"]["name"]=='baseitem.csv'){
		...
		$linea= str_replace('"',"", $linea);
		$col=explode(";",$linea);
		$orden1="INSERT INTO $BASEITEM VALUES ('','".$col[0]."','".$col[1]."','".$col[2]."','".$col[3]."','".$col[4]."','".$col[5]."')";
		}
	...
	if(mysql_query($orden1)){echo "El archivo " .$_FILES["archivo"]["name"]." fue cargado correctamente. La tabla ha sido actualizada.  ";}
